...
## 4. postprocess 결과 형식 (pred_dict)

```python
# src/engine/postprocess.py 반환값
...
> `src/engine/predict.py`는 raw model output(`Any`)을 반환. NMS/confidence 필터링 후 이 형식으로 변환하는 것은 `src/engine/postprocess.py` 책임 (ADR-006)
...
| 2026-05-21 | Section 4 — predict.py → postprocess.py 반환값으로 수정 (ADR-006 반영) | 김범진 |
