...
| 날짜   | 버전   | 모델      | 변경사항                                                                                                                                                                                   | val mAP@50 | Kaggle Score | 비고  | Kaggle Score |
| ---- | ---- | ------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- | ------------ | --- | ------------ |
| -    | v1.0 | YOLOv8n | 파이프라인 첫 완성                                                                                                                                                                             | -          | 0.17         | 제출  | 0.17         |
| 5.26 | v2.1 | YOLOv8n | transforms 증강, ultralytics 모자이크 증강, 학습할때 페이즈별 (페이즈1: head의 cv3 마지막 레이어만 파인튜닝, 페이즈2:헤드만 파인튜닝, 페이즈3: 백본 + 넥 + 헤드(lr을 낮게줘서 파인튜닝)), 각 페이즈마다 lr 스케줄링, adamw(cousin, momentum, weight decay) | 0.5053     | 0.53967      |     | 0.53967      |


|      | v2.2 | YOLOv8n | 모자이크 없앰 (0.0)                                                                                                                                                                          |            |              |     |
