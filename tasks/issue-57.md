...
## 2026-05-28 YOLOv8m RAW max detections 비교

YOLOv8m RAW checkpoint와 `class_agnostic_nms=True`, `conf_threshold=0.25`, `iou_threshold=0.70` 조건을 고정한 뒤 `max_detections`만 변경해 시각화 결과를 비교했다.

비교 조건:

| 조건 | checkpoint | conf | iou | max_detections | class_agnostic_nms | 결과 | 판단 |
|---|---|---:|---:|---:|---|---|---|
| maxdet04 | `best_model_YJY.pt` | 0.25 | 0.70 | 4 | true | 기준 이미지 5장 모두 동일 | 현재 기본값 유지 |
| maxdet08 | `best_model_YJY.pt` | 0.25 | 0.70 | 8 | true | 기준 이미지 5장 모두 동일 | 추가 bbox 없음 |
| maxdet20 | `best_model_YJY.pt` | 0.25 | 0.70 | 20 | true | 기준 이미지 5장 모두 동일 | 추가 bbox 없음 |

### 판단

- 이번 5장 시각화에서는 `max_detections`를 4, 8, 20으로 바꿔도 결과가 완전히 동일했다.
- NMS 이후 남는 prediction이 이미 4개 이하라 `max_detections`가 결과를 잘라내는 상황은 아니었다.
- 따라서 `max_detections=4`는 성능 개선용이라기보다 프로젝트 조건인 이미지당 최대 4개 알약을 보장하는 안전장치로 유지한다.
- 현재 샘플 5장에서는 `conf_threshold`, `iou_threshold`, `max_detections` 변경 모두 결과 차이가 없었으므로, 다음 실험은 `bbox clamp`, `max_box_area_ratio`, `TTA`처럼 다른 성격의 후처리/추론 전략을 확인한다.

## 2026-05-28 YOLOv8m RAW clamp / max box area 비교

YOLOv8m RAW checkpoint와 `class_agnostic_nms=True`, `conf_threshold=0.25`, `iou_threshold=0.70`, `max_detections=4` 조건을 고정한 뒤 bbox clamp와 큰 bbox 제거 조건을 비교했다.

비교 조건:

| 조건 | checkpoint | clamp | max_box_area_ratio | 결과 | 판단 |
|---|---|---|---:|---|---|
| base | `best_model_YJY.pt` | false | null | 기준 이미지 5장 모두 동일 | 기준 |
| clamp | `best_model_YJY.pt` | true | null | 기준 이미지 5장 모두 동일 | 변화 없음 |
| clamp_area025 | `best_model_YJY.pt` | true | 0.25 | 기준 이미지 5장 모두 동일 | 변화 없음 |
| clamp_area015 | `best_model_YJY.pt` | true | 0.15 | 기준 이미지 5장 모두 동일 | 변화 없음 |

### 판단

- 이번 5장 시각화에서는 bbox clamp와 max box area filtering을 적용해도 결과가 완전히 동일했다.
- 현재 YOLOv8m RAW + `class_agnostic_nms=True` 조합에서는 bbox가 이미지 밖으로 나가거나, 이미지 대부분을 덮는 큰 bbox가 보이지 않았다.
- `bbox clamp`는 제출 안정성용 방어 로직으로는 의미가 있지만, 이번 샘플에서는 실제 수정 효과가 없었다.
...
## 2026-05-28 YOLOv8m RAW 최종 후보 20장 검증

앞선 실험에서 선택한 후처리 후보를 train 샘플 20장으로 확장해 시각화 검증했다.
...
| 이미지 밖 bbox | 확인되지 않음 |
| 중복 bbox | 확인되지 않음 |
| GT와 bbox 위치 | 대부분 잘 맞음 |
| score | 대부분 0.9 이상, 일부 0.79~0.89 |

낮은 score 예시:

| image_id | 예측 | score | 판단 |
|---:|---|---:|---|
| 793 | P8 | 0.79 | 위치 정상 |
| 285 | P49 | 0.85 | 위치 정상 |
| 1001 | P15 | 0.86 | 위치 정상 |
...
- 20장 기준으로 현재 후처리 후보는 큰 이상 bbox, 중복 bbox, 누락 없이 안정적으로 동작했다.
- 일부 score가 0.9보다 낮은 정상 예측이 있어 `conf_threshold`를 과하게 올리면 recall이 떨어질 수 있다.
- 따라서 `conf_threshold=0.25`를 유지하는 것이 적절하다.
- 후처리 v0.2 기준 1차 후보는 아래 조합으로 정리한다.
...
- Kaggle 제출 비교는 진행하지 않았고, train 샘플 시각화 기준 검증이다.
